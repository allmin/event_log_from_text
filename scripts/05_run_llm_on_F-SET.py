...
def extract_events_funct(sentences, extractor=None, evidence={'keywords':[],'event_names':[],'similarities':[]}, keyword_output=None, phrase_output=None):
    global event_description_dict_llm, event_types
    events = extractor.extract_events(sentences=sentences, event_names=event_types, event_descriptions=event_description_dict_llm, threshold=0.2, prompt_evidence=evidence, keyword_output=keyword_output, phrase_output=phrase_output)
    return events
...
for keyword_output, phrase_output in [i for i in product([False,True],[False,True])]:
    for ET in event_types:
        ...
        for keyword_input, embedder_input in [i for i in product([False,True],[False,True])]:
            ...
            disagreement_df_temp.loc[:,f"LLM_Events_{col_suffix}_evidence"] = extract_events_funct(disagreement_df_temp.Sentence_dictionary,                                                                        evidence=evidence})
